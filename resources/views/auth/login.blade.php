<x-guest-layout :fixed="true">
    ...
    <!-- Session Status -->
    <x-auth-session-status class="mb-4 bg-neon/10 border border-neon/20 text-neon p-4 rounded-2xl text-xs font-bold" :status="session('status')" />
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-4">
        ...
        <div class="space-y-2">
            ...
            <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username" 
                class="block w-full bg-dark border border-white/5 focus:border-neon focus:ring-1 focus:ring-neon rounded-2xl p-4 text-white font-bold transition-all placeholder:text-gray-700 outline-none"
                placeholder="NAME@DOMAIN.COM">
            <x-input-error :messages="$errors->get('email')" class="mt-1 text-red-500 text-[10px] font-bold uppercase tracking-widest" />
        </div>
        ...
        <div class="space-y-2">
            ...
            <input id="password" type="password" name="password" required autocomplete="current-password" 
                class="block w-full bg-dark border border-white/5 focus:border-neon focus:ring-1 focus:ring-neon rounded-2xl p-4 text-white font-bold transition-all placeholder:text-gray-700 outline-none"
                placeholder="••••••••">
            <x-input-error :messages="$errors->get('password')" class="mt-1 text-red-500 text-[10px] font-bold uppercase tracking-widest" />
        </div>
        ...
        <div class="pt-2">
            <button type="submit" class="w-full bg-neon text-dark py-4 rounded-[2rem] font-black text-base uppercase tracking-tighter hover:scale-[1.02] transition shadow-[0_0_30px_rgba(190,242,100,0.3)] flex justify-center items-center group active:scale-95">
                Secure Access
