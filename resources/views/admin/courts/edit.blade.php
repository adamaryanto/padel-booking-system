...
<div class="max-w-2xl p-6 bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-800 dark:border-gray-700">
    <div class="flex items-center mb-6 pb-4 border-b border-gray-100 dark:border-gray-700">
        <i data-lucide="edit" class="w-5 h-5 text-primary-500 me-2"></i>
        <h3 class="text-lg font-bold text-gray-900 dark:text-white">Formulir Edit Lapangan</h3>
    </div>
    
    <form action="{{ route('admin.courts.update', $court) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')
        
        <div>
            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Lapangan</label>
            <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('name') border-red-500 @enderror" value="{{ old('name', $court->name) }}" required>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">{{ old('description', $court->description) }}</textarea>
            @error('description') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="photo">Foto Lapangan (Optional)</label>
            
            @if($court->photo)
                <div class="mb-4 p-2 bg-gray-50 dark:bg-gray-900 rounded-lg border border-dashed border-gray-300 dark:border-gray-600 inline-block">
                    <img src="{{ asset('storage/' . $court->photo) }}" alt="{{ $court->name }}" class="h-32 rounded shadow-sm">
                </div>
            @endif
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-300">Pilih file baru jika ingin mengubah. Max: 2MB, Format: jpg, png, jpeg, webp</p>
        ...
        <div>
            ...
            <select name="is_active" id="is_active" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                <option value="1" {{ old('is_active', $court->is_active) == '1' ? 'selected' : '' }}>Aktif</option>
